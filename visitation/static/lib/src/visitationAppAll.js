odoo.define("visitation.visitationAppIO", function (require) {
  ...
  class IO {
    ...
    static fetchBeds = async (session) => {
      const bedResult = await session.searchRead(
        "x_resident_bed",
        [],
        ["id", "x_unit_id", "x_room_id", "x_bed_position"]
      );
      if (bedResult.data.result) {
    ...
    static fetchContent = async (session) => {
      const contentResult = await session.searchRead(
        "x_visitation_content",
        [],
        ["x_key", "x_value"]
      );
      if (contentResult.data.result) {
    ...
    static fetchAvailabilities = async (session, id) => {
      const visitRequestRaw = await session.searchRead(
        "x_visit_request",
        [["id", "=", id]],
        ["x_availability_ids"]
      );
      const visitRequestAvailabilityIds =
        visitRequestRaw.data.result[0].x_availability_ids;
      const availabilityResult = await session.searchRead(
        "x_availability_slot",
    ...
    static updateRequestedAvailabilityId = async (
      session,
      id,
      availabilitySlotId
    ) => {
      return this.updateVisitRequest(session, id, {
...
odoo.define("visitation.visitationAppMain", function (require) {
  ...
  class VisitationApp extends Component {
    ...
    visitorFormSubmit = async (vals) => {
      const self = this;
      Object.assign(this.state.visitRequest, vals);
      await IO.updateVisitorScreenings(
        this.session,
        this.state.visitRequest.visitRequestId,
      ...
      const availabilitiesResult = await IO.fetchAvailabilities(
        this.session,
        this.state.visitRequest.visitRequestId
      ).catch((err) => {
        this.notify(err);
      });
      this.state.dataValues.availabilities = availabilitiesResult.data.result;
      this.stepForward();
    };
